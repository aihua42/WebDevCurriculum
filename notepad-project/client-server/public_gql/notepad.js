class Notepad {
  ...
  async #loadPrevTabs() {  // page reload 할때마다 실행된다. 
    ...
    const url2 = 'https://localhost:8080/graphql';
    ...
    this.textContainer.setPrevTabs(prevTabs.data.tabs);
  }
}
...
const onclickFuncMap = {
  ...
  async login() {
    const url = 'https://localhost:3030/login';
  ...
  async logout(textContainer, pageToGo='welcome') {
    ...
    const query = `
      mutation Logout($userId: String!, $activeTitle: String!, $tabs: [TitleNTextInput]!) {
        logout(userId: $userId, activeTitle: $activeTitle, tabs: $tabs)
      }
    `;

    const urlLogout = 'https://localhost:8080/graphql';
    ...
    .then((res) => {  
      ...
      let urlToGo = '';
      if (pageToGo === 'login') {
        urlToGo = 'https://localhost:3030/login';
      } else {
        urlToGo = 'https://localhost:3030';
      }
...
async function refreshAccessToken(textContainer) {
  ...
  try {
    const query = `
      mutation RefreshAccessToken($userId: String!, $refreshToken: String!) {
        refreshAccessToken(userId: $userId, refreshToken: $refreshToken)
      }
    `;
...
async function fetchGetText(title, textContainer) {
  ...
  try {
    const query = `
      query Text($userId: String!, $textId: String!) {
        text(userId: $userId, textId: $textId) {
          userId
          textId
          title
          text
    ...
    const res = await fetchAfterTokenRefreshed(url, {
      ...
      body: JSON.stringify({
        query,
        variables: { userId, textId }
      }),
    }, textContainer);  
    console.log("res from GET: ", res);

    if (!res.ok) {
      console.error('Error during fetchGet with status code: ', res.errors);
    } else {
...
async function fetchPostText(title, text, textContainer) {
  ...
  const query = `
    mutation AddText($userId: String!, $textId: String!, $title: String!, $text: String) {
      addText(userId: $userId, textId: $textId, title: $title, text: $text)
    }
  `;
  ...
  await fetchAfterTokenRefreshed(url, {
    ...
    body: JSON.stringify({
      query,
      variables: { userId, textId, title, text }
    }),
  }, textContainer)
  .then((res) => {
    if (res.status === 409) {
...
async function fetchPatchText(textId, key, preNnewVals, textContainer) {
  ...
  try {
    const query = `
      mutation UpdateText($userId: String!, $textId: String!, $key: String!, $before: String!, $after: String!) {
        updateText(userId: $userId, textId: $textId, key: $key, before: $before, after: $after)
      }
    `;
...
async function fetchDeleteText(title, textContainer) {
  ...
  try {
    const query = `
      mutation DeleteText($userId: String!, $textId: String!) {
        deleteText(userId: $userId, textId: $textId)
      }
    `;
